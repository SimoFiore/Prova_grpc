...
type RouteGuideClient interface {
	...
	// Aggiungo la prova per il db
	DBmanager(ctx context.Context, in *SessionID, opts ...grpc.CallOption) (RouteGuide_DBmanagerClient, error)
}
...
func (c *routeGuideClient) DBmanager(ctx context.Context, in *SessionID, opts ...grpc.CallOption) (RouteGuide_DBmanagerClient, error) {
	stream, err := c.cc.NewStream(ctx, &RouteGuide_ServiceDesc.Streams[3], "/routeguide.RouteGuide/DBmanager", opts...)
	if err != nil {
		return nil, err
	}
	x := &routeGuideDBmanagerClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type RouteGuide_DBmanagerClient interface {
	Recv() (*Samples, error)
	grpc.ClientStream
}

type routeGuideDBmanagerClient struct {
	grpc.ClientStream
}

func (x *routeGuideDBmanagerClient) Recv() (*Samples, error) {
	m := new(Samples)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type RouteGuideServer interface {
	...
	// Aggiungo la prova per il db
	DBmanager(*SessionID, RouteGuide_DBmanagerServer) error
...
func (UnimplementedRouteGuideServer) DBmanager(*SessionID, RouteGuide_DBmanagerServer) error {
	return status.Errorf(codes.Unimplemented, "method DBmanager not implemented")
}
...
func _RouteGuide_DBmanager_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(SessionID)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(RouteGuideServer).DBmanager(m, &routeGuideDBmanagerServer{stream})
}

type RouteGuide_DBmanagerServer interface {
	Send(*Samples) error
	grpc.ServerStream
}

type routeGuideDBmanagerServer struct {
	grpc.ServerStream
}

func (x *routeGuideDBmanagerServer) Send(m *Samples) error {
	return x.ServerStream.SendMsg(m)
}
...
var RouteGuide_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "DBmanager",
			Handler:       _RouteGuide_DBmanager_Handler,
			ServerStreams: true,
		},
	},
